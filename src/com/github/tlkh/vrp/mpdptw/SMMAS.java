...
import com.github.tlkh.vrp.mpdptw.aco.SequentialInfeasible;
import com.github.tlkh.vrp.mpdptw.aco.SolutionBuilder;
import com.github.tlkh.vrp.mpdptw.alns.InsertionHeuristic;
...
public class SMMAS {
    ...
    public SMMAS(ProblemInstance instance) {
    ...
    public void evaporation() {
        for (int i = 0; i < pheromoneNodes.length; i++) {
            for (int j = 0; j < pheromoneNodes[i].length; j++) {
                if (i != j) {
                    pheromoneNodes[i][j] = (1.0 - rho) * pheromoneNodes[i][j] + rho * trailMin;
                }
            }
        }
    }
    ...
    public void globalPheromoneDeposit(Solution ant) {
        ...
        for (int i = 0; i < ant.tours.size(); i++) {
            for (int j = 0; j < ant.tours.get(i).size() - 1; j++) {
                ...
                pheromoneNodes[from][to] = pheromoneNodes[from][to] + rho* trailMax;
